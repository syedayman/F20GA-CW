...
glm::vec4 ia = glm::vec4(0.5f, 0.5f, 0.5f, 0.5f);
glm::vec4 is = glm::vec4(0.5f, 0.5f, 0.5f, 0.5f);
glm::vec4 id = glm::vec4(0.5f, 0.5f, 0.5f, 0.5f);
glm::vec4 lightPos = glm::vec4(1.0f, 1.0f, 1.0f, 2.0f);
...
int main()
{
	...
	//const auto title = "My 3D Graphics and Animation OpenGL Application"s;
...
void startup()
{
	...
	content.LoadGLTF("assets/middle.gltf");
...
void update()
{
	if (keyStatus[GLFW_KEY_LEFT]) cameraPosition.x -= 0.1f;
	if (keyStatus[GLFW_KEY_RIGHT]) cameraPosition.x += 0.1f;
	if (keyStatus[GLFW_KEY_UP]) cameraPosition.y += 0.1f;
	if (keyStatus[GLFW_KEY_DOWN]) cameraPosition.y -= 0.1f;
	if (keyStatus[GLFW_KEY_W]) modelPosition.z += 0.5f;
	if (keyStatus[GLFW_KEY_S]) modelPosition.z -= 0.5f; 
...
void onMouseMoveCallback(GLFWwindow *window, double x, double y)
{
	...
	if (firstMouse) {
	lastX = (float)mouseX; lastY = (float)mouseY; firstMouse 
	= false;
	}
	...
	if (keyStatus[GLFW_KEY_A]) cameraPosition -= 
	glm::normalize(glm::cross(cameraFront, cameraUp)) * cameraSpeed;
	if (keyStatus[GLFW_KEY_D]) cameraPosition += 
	glm::normalize(glm::cross(cameraFront, cameraUp)) * cameraSpeed;
